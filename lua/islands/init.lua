...
local DEFAULT_CONFIG = {
   ...
   colors = require("islands.palette"),
   overrides = {},
   theme = "islands",
}
...
local function get_configs()
   ...
   if g.colors_name == "islands" then
      configs.theme = "islands"
      configs.colors = require("islands.palette")
   end
...
---setup islands colorscheme
---@param configs IslandsConfig?
local function setup(configs)
   if type(configs) == "table" then
      user_configs = configs --[[@as IslandsConfig]]
   end
end

---load islands colorscheme
---@param theme string?
local function load(theme)
   if vim.fn.has("nvim-0.7") ~= 1 then
      vim.notify("islands.nvim: you must use neovim 0.7 or higher")
      return
   end
   ...
   g.colors_name = theme or "islands"
